...
class AlignedSpinTemplate(object):
    ...
    * Provide a classmethod from_sim, which creates an instance based on
    ...
    """
    approximant = None
    __slots__ = ("m1", "m2", "spin1z", "spin2z", "chieff", "bank", "_dur",
                 "_mchirp", "_tau0", "_wf", "_metric", "sigmasq",
    ...
    def optimize_flow(self, flow_min, fhigh_max, noise_model, df=0.1,
                      sigma_frac=0.99):
        """Set the template's flow as high as possible but without losing
        more than a given fraction of the template's sigma when calculated
    ...
    @classmethod
    def from_hdf(cls, hdf_fp, idx, bank):
        return cls(float(hdf_fp['mass1'][idx]), float(hdf_fp['mass2'][idx]),
    ...
    def brute_match(self, other, df, workspace_cache, **kwargs):
        return InspiralSBankComputeMatch(
                self.get_whitened_normalized(df, **kwargs),
...
class EOBNRv2Template(SEOBNRv2Template):
    ...
    def __init__(self, m1, m2, bank, flow=None):
...
class PrecessingSpinTemplate(AlignedSpinTemplate):
    ...
    def __init__(self, m1, m2, spin1x, spin1y, spin1z, spin2x, spin2y, spin2z, theta, phi, iota, psi, orb_phase, bank, flow=None):
    ...
    @classmethod
    def from_hdf(cls, hdf_fp, idx, bank):
        return cls(hdf_fp['mass1'][idx], hdf_fp['mass2'][idx],
                   hdf_fp['spin1x'][idx], hdf_fp['spin1y'][idx],
                   hdf_fp['spin1z'][idx], hdf_fp['spin2x'][idx],
                   hdf_fp['spin2y'][idx], hdf_fp['spin2z'][idx],
                   hdf_fp['latitude'][idx], hdf_fp['longitude'][idx],
                   hdf_fp['polarization'][idx], hdf_fp['inclination'][idx],
                   hdf_fp['orbital_phase'][idx], bank,
                   flow=hdf_fp['f_lower'][idx])
