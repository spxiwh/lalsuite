...
if opts.trial_waveforms:
    if opts.trial_waveforms.endswith(('.xml', '.xml.gz')):
        ...
        for idx in xrange(num_points):
            ...
            proposal.append(tmplt_class.from_hdf(hdf_fp, idx, bank))
        hdf_fp.close()
...
if opts.output_filename.endswith(('.xml', '.xml.gz')):
    ...
    if len(tbl) == 0:
        ...
        # Approximants need to be mapped to str
        approx_nums = set(tbl['approximant_id'])
        for approx_num in approx_nums:
            approx = lalsim.GetStringFromApproximant(approx_num)
            hdf_fp.attrs['approx_num_{}'.format(approx_num)] = approx
