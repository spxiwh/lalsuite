...
for tmplt in proposal:
    ...
    if match < opts.match_min:
        ...
        # Now add templates with identical params, but varying inclinations
        count = 0
        while count < 100:
            # Generate new template
            new_iota = uniform(0, np.pi)
            new_tmplt = tmplt_class(tmplt.m1, tmplt.m2, tmplt.spin1x,
                                    tmplt.spin1y, tmplt.spin1z, tmplt.spin2x,
                                    tmplt.spin2y, tmplt.spin2z, tmplt.theta,
                                    tmplt.phi, new_iota, tmplt.psi,
                                    tmplt.orb_phase, bank=bank)
            match2, matcher2 = bank.covers(new_tmplt, opts.match_min)
            if match2 < opts.match_min:
                bank.insort(new_tmplt)
                count=0
            else:
                count = count + 1
